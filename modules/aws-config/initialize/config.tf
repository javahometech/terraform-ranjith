...
# Create S3 bucket to store AWS config output data
module "config-s3-bucket" {
  source               = "git::https://github.optum.com/CommercialCloud-EAC/aws_s3.git//terraform_module/simple?ref=v1.2.0"
  bucket_name          = "${data.template_file.config.vars.s3_bucket_name}"
  bucket_force_destroy = "${data.template_file.config.vars.config_force_destroy}"
  name_space           = "${data.template_file.config.vars.name_space}"
  s3_tags              = "${var.config_tags}"
...
resource "aws_config_delivery_channel" "config" {
  name           = "${data.template_file.config.vars.config_name}"
  s3_bucket_name = "${module.config-s3-bucket.bucket_id}"
